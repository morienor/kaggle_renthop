...
class desc_ft_generator(object):
    ...
    def __init__(self, type, out_dir, keep_tokens= 300, las_keep_components = 100, save_output=True):
        self.type = type
        self.out_dir = out_dir
        self.save_output = save_output
        self.keep_tokens = keep_tokens
        self.las_keep_components = las_keep_components
    ...
    def gen_lsa(self):
        ...
        self.dictionary.filter_extremes(no_below=10, keep_n= self.keep_tokens)
        ...
        # ******* LSA *******
        self.corpus_lsa = lsa(n_components = self.las_keep_components).fit_transform(X=corpus_tfidf.T)
...
def get_description_features(argv):
	# this is just my attempt to run this from jupyther - not working
    
    args = {"type":" ","out_dir":" ","save_output":" ", "keep_tokens": " ", "las_keep_components": " " }
    
    args["type"] = argv[0]
    args["out_dir"] = argv[1] 
    args["save_output"] = argv[2]
    args["keep_tokens"] = argv[3]
    args["las_keep_components"] = argv[4]

      
    gen = desc_ft_generator(type = args["type"], out_dir = args["out_dir"], save_output=args["save_output"], 
    keep_tokens=args["keep_tokens"], las_keep_components =  args["las_keep_components"])
